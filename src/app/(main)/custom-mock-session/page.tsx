...
import { ArrowRight, Compass, Crown, Info, PlayCircle, RotateCcw, Search, ShieldCheck, SlidersHorizontal, Sparkles, Target, TrendingUp, UserRound } from 'lucide-react';
...
export default function StartPracticeSessionPage() {
  ...
  const filterReady = !!examTypeId && !!classId;
  ...
  const heroPrimaryButtonClass =
    'h-11 rounded-full border border-cyan-300/40 bg-gradient-to-r from-cyan-300 via-sky-400 to-blue-500 px-5 font-semibold text-slate-950 shadow-[0_10px_24px_-12px_rgba(56,189,248,0.9)] transition-all duration-300 hover:translate-y-[-1px] hover:from-cyan-200 hover:via-sky-300 hover:to-blue-400 hover:shadow-[0_14px_30px_-12px_rgba(56,189,248,1)]';
  const teacherActionPrimaryClass =
    'h-10 w-full rounded-xl border border-cyan-300/35 bg-gradient-to-r from-cyan-300 via-sky-400 to-blue-500 px-3 text-sm font-semibold text-slate-950 transition-all hover:from-cyan-200 hover:via-sky-300 hover:to-blue-400';
  const teacherActionPremiumClass =
    'h-10 w-full rounded-xl border border-amber-400/45 bg-amber-500/10 px-3 text-sm font-semibold text-amber-100 transition-all hover:bg-amber-500/20 hover:text-amber-50';
  ...
  return (
    <div className="flex flex-col h-full">
      <DashboardHeader title="Custom Mock Test" />
      <main className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8">
        <div className="space-y-6">
          <Card className="relative isolate overflow-hidden border-primary/35 bg-gradient-to-br from-emerald-950/45 via-teal-950/35 to-cyan-950/45 shadow-[0_24px_54px_-34px_rgba(20,184,166,0.72)]">
            <div className="pointer-events-none absolute right-[-95px] top-[-95px] h-56 w-56 rounded-full bg-teal-400/25 blur-3xl" />
            <div className="pointer-events-none absolute bottom-[-95px] left-[-95px] h-56 w-56 rounded-full bg-cyan-400/25 blur-3xl" />
              ...
              <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
                <div className="space-y-4">
                  <p className="inline-flex w-fit items-center gap-2 rounded-full border border-primary/30 bg-primary/10 px-3 py-1 text-xs font-medium text-primary">
                    ...
                    Personalized Mock Engine
                  </p>
                  <h2 className="text-2xl font-semibold tracking-tight md:text-3xl">Create Precision Mock Tests That Feel Like the Real Exam</h2>
                  <p className="max-w-2xl text-sm text-muted-foreground">
                    Pick your exam and curriculum, get teacher matches from relevant question banks, then build custom mocks aligned to your target score.
                  </p>
                  <div className="grid grid-cols-1 gap-2 text-xs sm:grid-cols-3">
                    <div className="rounded-lg border border-cyan-300/25 bg-cyan-500/10 px-3 py-2">
                      <p className="font-semibold text-cyan-100">Step 1</p>
                      <p className="mt-0.5 text-muted-foreground">Select exam + curriculum</p>
                    </div>
                    <div className="rounded-lg border border-sky-300/25 bg-sky-500/10 px-3 py-2">
                      <p className="font-semibold text-sky-100">Step 2</p>
                      <p className="mt-0.5 text-muted-foreground">Choose subject focus</p>
                    </div>
                    <div className="rounded-lg border border-emerald-300/25 bg-emerald-500/10 px-3 py-2">
                      <p className="font-semibold text-emerald-100">Step 3</p>
                      <p className="mt-0.5 text-muted-foreground">Build free or premium mock</p>
                    </div>
                  </div>
                </div>
                <div className="rounded-2xl border border-cyan-300/20 bg-slate-950/35 p-4">
                  <p className="mb-3 text-xs font-semibold uppercase tracking-[0.14em] text-cyan-200">Journey Map</p>
                  <div className="space-y-2.5">
                    <div className="flex items-start gap-2 rounded-lg border border-white/10 bg-slate-950/40 p-2.5">
                      ...
                      <p className="text-xs text-slate-200/90">Set exam and curriculum for precise teacher matching.</p>
                    </div>
                    <div className="flex items-start gap-2 rounded-lg border border-white/10 bg-slate-950/40 p-2.5">
                      <Target className="mt-0.5 h-4 w-4 text-sky-300" />
                      <p className="text-xs text-slate-200/90">Add subject focus to optimize practice relevance.</p>
                    </div>
                    <div className="flex items-start gap-2 rounded-lg border border-white/10 bg-slate-950/40 p-2.5">
                      <TrendingUp className="mt-0.5 h-4 w-4 text-emerald-300" />
                      <p className="text-xs text-slate-200/90">Launch free or premium mock plans with one click.</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <Card className="border-border/80 bg-gradient-to-b from-card/90 via-card/80 to-slate-950/25">
            <CardHeader className="pb-3">
              <CardTitle className="flex items-center gap-2 text-xl md:text-2xl">
                ...
                Build Your Mock Blueprint
              </CardTitle>
              <CardDescription>Set your target exam profile and unlock the best teacher-matched mock building flow.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-5">
              {loading ? (
                ...
                <>
                  <div className="grid grid-cols-1 gap-2 text-xs sm:grid-cols-3">
                    <div className="rounded-lg border border-border/70 bg-slate-900/50 px-3 py-2">
                      <p className="text-muted-foreground">Exam</p>
                      <p className="mt-1 font-semibold text-foreground">{selectedExamName || 'Not selected'}</p>
                    </div>
                    <div className="rounded-lg border border-border/70 bg-slate-900/50 px-3 py-2">
                      <p className="text-muted-foreground">Curriculum</p>
                      <p className="mt-1 font-semibold text-foreground">{selectedClassName || 'Not selected'}</p>
                    </div>
                    <div className="rounded-lg border border-border/70 bg-slate-900/50 px-3 py-2">
                      <p className="text-muted-foreground">Subject Focus</p>
                      <p className="mt-1 font-semibold text-foreground">{selectedSubjectIds.length > 0 ? `${selectedSubjectIds.length} selected` : 'All available'}</p>
                    </div>
                  </div>
                  ...
                  <div className={cn(
                    ...
                    <p className="text-[11px] font-semibold uppercase tracking-[0.13em] text-muted-foreground">Current Setup</p>
                    <div className="mt-2 flex flex-wrap gap-2">
                      ...
                      <Badge variant="outline" className="border-border/70 bg-background/35">
                        {selectedSubjectIds.length > 0 ? `${selectedSubjectIds.length} subject filters` : 'All subjects'}
                      </Badge>
                    </div>
                    <p className="mt-2 text-xs text-muted-foreground">
                      {hasPendingFilterChanges ? 'Filters changed. Click unlock to refresh teacher matches.' : 'Teacher results are synced with your current setup.'}
                    </p>
                  </div>
                  ...
                  <div className="rounded-lg border border-border/70 bg-muted/20 p-4">
                    <div className="mb-3 flex items-center justify-between">
                      <p className="text-sm font-semibold">Subjects (optional but recommended)</p>
                      <Badge variant="outline" className="rounded-full">{selectedSubjectIds.length} selected</Badge>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                      {filteredSubjects.length === 0 ? (
                        <p className="text-sm text-muted-foreground md:col-span-3">Select exam and curriculum to unlock subject-level filtering.</p>
                      ) : (
                    ...
                    <div className="rounded-xl border border-amber-300/35 bg-gradient-to-r from-amber-500/15 via-amber-500/8 to-transparent p-3">
                      <div className="flex items-start gap-2.5">
                        ...
                        <div>
                          ...
                          <p className="mt-0.5 text-xs text-amber-100/85">
                            Subject-level mock setup is currently updating. Try another curriculum for instant mock creation, or revisit soon.
                          </p>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <Button type="button" className={heroPrimaryButtonClass} disabled={!filterReady} onClick={() => setAppliedFilters({ examTypeId, classId, subjectIds: selectedSubjectIds })}>
                      <Search className="mr-2 h-4 w-4" />
                      Unlock Best Matching Teachers
                    </Button>
                  </div>
                </>
              )}
          ...
          {appliedFilters ? (
            <Card className="border-border/70 bg-gradient-to-r from-slate-950/90 via-cyan-950/20 to-emerald-950/20">
              <CardContent className="flex flex-col gap-4 p-4 md:flex-row md:items-center md:justify-between">
                <div>
                  <p className="text-[11px] font-semibold uppercase tracking-[0.13em] text-muted-foreground">Match Snapshot</p>
                  <p className="mt-1 text-lg font-semibold">{matchedTeachers.length} teacher{matchedTeachers.length === 1 ? '' : 's'} matched</p>
                  <p className="text-xs text-muted-foreground">Use Free Mock for quick practice or Premium for deeper targeted drills.</p>
                </div>
          ...
          {!appliedFilters ? (
            <Card className="border-dashed border-primary/30 bg-primary/5">
              <CardContent className="p-10 text-center">
                ...
                <p className="font-medium">Ready to craft your high-impact mock session</p>
                <p className="mt-1 text-sm text-muted-foreground">Apply filters to discover teacher matches and start building your personalized mock strategy.</p>
              </CardContent>
            </Card>
          ) : questionsLoading ? (
            ...
            <Card className="border-dashed border-amber-500/30 bg-amber-500/5">
              <CardContent className="p-10 text-center">
                ...
                <p className="font-medium">No high-fit teacher match found for this setup</p>
                <p className="mt-1 text-sm text-muted-foreground">Try broadening subject filters or switching curriculum to unlock more question-bank coverage.</p>
                <div className="mt-4 flex justify-center gap-2">
                  <Button type="button" variant="outline" onClick={() => setSelectedSubjectIds([])}>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <Badge
                              variant="outline"
                              className="shrink-0 border-emerald-300/35 bg-emerald-500/10 px-2 py-0.5 text-[10px] font-semibold text-emerald-100"
                            >
                              Recommended
                            </Badge>
                          </div>
                          <p className="mt-1 text-xs leading-5 text-muted-foreground line-clamp-2">
                            Best-fit teacher for your selected exam, curriculum, and subject filters.
                          </p>
                        </div>
                      </div>

                      <div className="mt-3 grid grid-cols-2 gap-2">
                        <Button asChild size="sm" className={teacherActionPrimaryClass}>
                          <Link
                            onClick={(e) => e.stopPropagation()}
                            href={`/custom-mock-tests/create?teacherId=${encodeURIComponent(t.id)}&examTypeId=${encodeURIComponent(appliedFilters.examTypeId)}&classId=${encodeURIComponent(appliedFilters.classId)}&subjectIds=${encodeURIComponent(appliedFilters.subjectIds.join(','))}&examTypeName=${encodeURIComponent(selectedExamName)}&className=${encodeURIComponent(selectedClassName)}`}
                          >
                            <span className="truncate">Free Mock</span>
                            <ArrowRight className="ml-1.5 h-4 w-4 shrink-0" />
                          </Link>
                        </Button>
                        <Button asChild size="sm" variant="outline" className={teacherActionPremiumClass}>
                          <Link
                            onClick={(e) => e.stopPropagation()}
                            href={`/premium-mock-test/${encodeURIComponent(t.id)}?examTypeId=${encodeURIComponent(appliedFilters.examTypeId)}&classId=${encodeURIComponent(appliedFilters.classId)}&subjectIds=${encodeURIComponent(appliedFilters.subjectIds.join(','))}&examTypeName=${encodeURIComponent(selectedExamName)}&className=${encodeURIComponent(selectedClassName)}`}
                          >
                            <span className="truncate">Premium</span>
                            <Crown className="ml-1.5 h-4 w-4 shrink-0" />
                          </Link>
                        </Button>
                      </div>

                      <div className="mt-2.5 flex items-center gap-2 border-t border-border/50 pt-2 text-[11px] text-muted-foreground">
                        <ShieldCheck className="h-3.5 w-3.5 text-cyan-300" />
                        <span className="truncate">Unlimited mock variants available for this profile</span>
                      </div>
                    </CardContent>
                  </Card>
                );
